...
namespace ioSoftSmiths.TileMap
{
    public static class Generate
    {
        ...
        private static ushort EmptyMaterial = 0;
        private static ushort WallMaterial = 1;
        private static ushort FloorMaterial = 2;

        private static TileMap2D m_Tilemap;
        public static bool GenerateDone = false;
        public static bool m_Generating = false;
        public static bool Generating { get { return m_Generating; } }
        ...
        public static class Settings
        {
            ...
            //Add routing cost to turns in tunnels (creates straighter tunnels when false)
            //internal static bool CurvyTunnels = false;
            public static TTCost TunnelTurningCost = TTCost.Normal;
            internal static float CalcTurnCost { get { return (float)TunnelTurningCost / 2f; } }

            public enum TTCost : int
            {
                High = 4,
                Normal = 2,
                Low = 1,
        ...
        //private static IEnumerator _ThreadDungeon()  //UNITY
        public static void Dungeon()
        {
            ...
            double tarAspect = Settings.AspectRatio;
            while (tarAspect < (1.0f / MAX_ASPECT))
                tarAspect = m_Random.NextDouble() * MAX_ASPECT;

            int roomCount = m_Random.Next(Settings.RoomCountMin, Settings.RoomCountMax);

            //Place rooms
            var roomBndys = new List<Bounds>();
            var mapBounds = new Bounds();
            var rooms = new RoomGroup();
            var mapOuterBuf = Settings.MapOuterBuffer;
            for (int bndyIdx = 0; bndyIdx < roomCount; ++bndyIdx)
            {
                ...
                var origin = new IVector2(mapOuterBuf, mapOuterBuf);
                ...
                var curAspect = (double)mapBounds.Width / (double)mapBounds.Height;

                //If too wide then pick random x to start else pick random y to start
                if (curAspect > tarAspect)
                {
                ...
                {
                    var overlaps = false;
                    for (int otherBndys = 0; otherBndys < bndyIdx; ++otherBndys)
                        if (curBnds.Overlaps(roomBndys[otherBndys])) overlaps = true;

                    if (!overlaps) break;
                    curBnds.Translate(curAspect > tarAspect ? IVector2.Yp : IVector2.Xp);
                }
            ...
            //Create new Tilemap size based on room placement bounds
            m_Tilemap = new TileMap2D(new IVector2(mapBounds.Width, mapBounds.Height)) { m_Rooms = rooms };

            logMsg = "Done placing rooms.  Creating Tunnels.";
            Msg.Log(LOGKEY_MESSAGES, LOGKEY_MESSAGES, logMsg, MsgPriLvl.HIGH);

            //2.  Add connectors/Tunnels and Draw to map ====================================================================
            m_Tilemap.m_EntryPoint = new IVector2(m_Random.Next(mapOuterBuf, m_Tilemap.Dims.x),
                m_Random.Next(mapOuterBuf, m_Tilemap.Dims.y));
            ...
            //Draw Tunnel walls
            foreach (TunnelNetwork tunnelNet in m_Tilemap.m_Tunnels)
                foreach (TunnelNetwork.IPath path in tunnelNet.Paths)
                {
            ...
            //Draw Tunnel Floor
            foreach (TunnelNetwork tunnel in m_Tilemap.m_Tunnels)
                foreach (TunnelNetwork.IPath path in tunnel.Paths)
                    m_Tilemap.m_MapData.DrawPath(path.Points, (ushort)FloorMaterial);

            //Add Doorways
            AddDoorways();

            long timeStampEnd = DateTime.Now.Ticks / TimeSpan.TicksPerMillisecond;
        ...
        internal static void AddDoorways()
        {
            ...
            foreach (var path in roomConnectedPaths)
                for (int roomSelect = 0; roomSelect <= 1; ++roomSelect)
                {
                    ...
                    while (tunScanIdx > -1 && tunScanIdx < path.Points.Count)
                    {
                        ...
                        if (!m_Tilemap.InBounds(curPathCoord + IVector2.Xp))
                            ...
                            if (m_Tilemap[curPathCoord + IVector2.Xp] == WallMaterial)
                                xCount++;

                        if (!m_Tilemap.InBounds(curPathCoord + IVector2.Xn))
                            xCount++;
                        else
                            if (m_Tilemap[curPathCoord + IVector2.Xn] == WallMaterial)
                                xCount++;

                        if (!m_Tilemap.InBounds(curPathCoord + IVector2.Yp))
                            yCount++;
                        else
                            if (m_Tilemap[curPathCoord + IVector2.Yp] == WallMaterial)
                                yCount++;

                        if (!m_Tilemap.InBounds(curPathCoord + IVector2.Yn))
                            yCount++;
                        else
                            if (m_Tilemap[curPathCoord + IVector2.Yn] == WallMaterial)
                                yCount++;
        ...
        private static void CreateTunnels(IVector2 _entryPt) 
        {
            ...
            List<Triangule> tris = dt.Triangulate(); //TODO Creates null on seed 22
            if (tris == null)
            {
            ...
            ioGraph<IVector2> roomMST = roomGraph.PrimMST();


            //Get extra room connections beyond mst and add them to the graph

            double extTunnelPct = (Settings.ExtraTunnelMinPct +
                                   (Settings.ExtraTunnelMaxPct - Settings.ExtraTunnelMinPct) * m_Random.NextDouble());
            int remEdgeAddCnt = (int)((double)(roomGraph.EdgeCount - roomMST.EdgeCount) * extTunnelPct);
            ...
            {
                Msg.LogDebug(TAG_DEBUG,
                    "Additional Edge count (beyond MST) greater than available edges remaining.  Trimming", MsgPriLvl.HIGH);
                remEdgeAddCnt = remAvailEdges.Count;
            }

            for (int i = 0; i < remEdgeAddCnt; ++i)
            {
                var connection = remAvailEdges[m_Random.Next(remAvailEdges.Count)];

                ioGraph<IVector2>.ioGraphEdge edge = new ioGraph<IVector2>.ioGraphEdge(1, connection.v2);
            ...
            string logMsg = "Creating Tunnels";
            Msg.Log(LOGKEY_MESSAGES, LOGKEY_MESSAGES, logMsg, MsgPriLvl.HIGH);
            int edgesDone = 0;

            double debugRoutingTime = 0;
            double debugAStarTime = 0;
            for (int nodeIdx = 0; nodeIdx < roomMST.NodeCount; ++nodeIdx)
            {
                ...
                foreach (var edge in roomMST.GetConnectionsFast(nodeIdx))
                {
                    ...
                    logMsg = "Creating Tunnels " + (int)(((float)edgesDone / (float)roomMST.EdgeCount) * 100);
                    //Log(logMsg);
                    Msg.Log(LOGKEY_MESSAGES, LOGKEY_MESSAGES, logMsg, MsgPriLvl.HIGH);

                    var roomsToConnect = new Room[] { m_Tilemap.m_Rooms.Get(fromVector), m_Tilemap.m_Rooms.Get(toVector) };
                    ...
                    DateTime routingTimeStamp = DateTime.Now;
            ...
            bool connectEntry = true;
            int entryDist = int.MaxValue;

            //Connect entry point to closest room or tunnel
            foreach (Room room in m_Tilemap.m_Rooms)
            ...
            if (connectEntry)
            {
                ...
                var closestCoords = Support2D.GetNearestCoords(new HashSet<IVector2> { _entryPt }, availCoords, ref distance);
        ...
        {
            double defCost = _tunnelRoutingStrength * 2d + 1;
            ...
            foreach (var room in m_Tilemap.m_Rooms)
            {
                ...
                for (int buffer = _roomBuffer; buffer > 0; --buffer)
                {
            ...
            if (_tunnelMagBuffer >= 0 && Settings.CalcRoutingStrength != 0)
            {
                ...
                tunnelCost[0] = 1d;
                for (int tScaleIdx = 1; tScaleIdx <= _tunnelMagBuffer; ++tScaleIdx)
                    tunnelCost[tScaleIdx] = tunnelCost[0] + _tunnelRoutingStrength +
                                            (tScaleIdx - 1) * (defCost - 1 - _tunnelRoutingStrength) / _tunnelMagBuffer;
                foreach (TunnelNetwork tunnel in m_Tilemap.m_Tunnels)
                {
                    bool connects = true;
                    /*foreach (Room room in _exclude)
                {
                    if (tunnel.ConnectsTo(room)) connects = true;
                }*/

                    if (connects)
                        for (int tOffset = _tunnelMagBuffer; tOffset >= 0; --tOffset)
                        {
