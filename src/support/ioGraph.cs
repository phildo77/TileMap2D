...
namespace ioSoftSmiths.Collections
{
    ...
    /*public class GraphBiDir<T>
    {
        public const string TAG_DEBUG = "ioSoftSmiths.ioCollections.GraphBiDir";
        public struct GraphEdge
        {
            ...
            public bool ConnectionEquals(GraphEdge _other)
            {
                return From.Equals(_other.From) && To.Equals(_other.To) ||
                       From.Equals(_other.To) && To.Equals(_other.From);
            }

            public bool ConnectsNode(T _node)
            {
                return From.Equals(_node) || To.Equals(_node);
            }
        }
        ...
        public void AddEdge(T _from, T _to, double _weight)
        {
            var newEdge = new GraphEdge {From = _from, To = _to, Weight = _weight};
            for (int idx = 0; idx < m_Edges.Count; ++idx)
            {
                var curEdge = m_Edges[idx];
                if (curEdge.ConnectionEquals(newEdge))
                {
                    Msg.LogDebug(TAG_DEBUG,"Tried to add edge that already exists in graph: " + _from + " <=> " + _to + " - Doing Nothing.", MsgPriLvl.MED);
                    return;
                }
            }
            m_Edges.Add(newEdge);
        ...
        public List<T> GetConnections(T _node)
        {
            ...
            foreach (var edge in m_Edges)
            {
                if (edge.From.Equals(_node)) 
                    edges.Add(edge.To);
                else if (edge.To.Equals(_node))
                    edges.Add(edge.From);
            }
            return edges;
        }
        ...
        {
            return m_Edges.Any(edge => edge.ConnectsNode(_node));
        }
